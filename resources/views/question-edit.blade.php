...
@section('content')
    <section class="main-content max-w-150 translate-x-0 mx-auto my-15 duration-300">
        ...
        <form action="{{ route('question.edit') }}" method="post" class="w-full rounded-3xl">
            ...
            <div class="mb-8">
                <div class="relative">
                    <input type="text" name="title" id="title" placeholder=" " value="{{ $question->title }}"
            ...
            <textarea name="body" id="text" placeholder="Place here detalies(optional)"
                class="mb-8 border min-h-30 p-4 dark:border-neutral-600 focus:border-neutral-300 dark:focus:border-neutral-500 border-neutral-400 w-full rounded-2xl outline-none">{{ $question->body }}</textarea>
            @error('text')
                <span class="font-medium text-red-500">{{ $message }}</span>
            @enderror
